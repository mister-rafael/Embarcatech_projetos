...
#define WIFI_SSID "SR-OLIVER" // Substitua pelo nome da sua rede Wi-Fi
#define WIFI_PASS "Jw5|0390"  // Substitua pela senha da sua rede Wi-Fi
...
void create_http_response() {
    snprintf(http_response, sizeof(http_response),
        "HTTP/1.1 200 OK\r\nContent-Type: text/html; charset=UTF-8\r\n\r\n"
        "<!DOCTYPE html>"
        "<html><head><meta charset=\"UTF-8\"><title>Controle</title>"
        "<style>"
        "body{font-family:sans-serif;background:#f2f2f2;text-align:center;padding:20px;}"
        "h1{color:#333;}"
        ".b{display:inline-block;padding:10px 20px;margin:5px;font-size:16px;"
        "color:#fff;border:none;border-radius:8px;text-decoration:none;}"
        ".on{background:#4CAF50;}.off{background:#f44336;}.u{background:#2196F3;}"
        "</style></head><body>"
        "<h1>Controle do LED</h1>"
        "<a href=\"/led/on\" class=\"b on\">Ligar</a>"
        "<a href=\"/led/off\" class=\"b off\">Desligar</a>"
        "<a href=\"/update\" class=\"b u\">Atualizar</a>"
        "<p>Botão 1: %s</p><p>Botão 2: %s</p>"
        "</body></html>\r\n",
        button1_message, button2_message);
}
...
static err_t http_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err)
{
    ...
    else if (strstr(request, "GET /status"))
    {
        snprintf(http_response, sizeof(http_response),
                 ...
                 "{\"botao1\":\"%s\",\"botao2\":\"%s\",\"led\":%d}",
                 button1_message, button2_message, gpio_get(LED_PIN));
    }
    else
    {
        snprintf(http_response, sizeof(http_response),
